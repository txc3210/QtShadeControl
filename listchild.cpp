...
void ListChild::tableInit()
{
    ...
    foreach (auto controller, *list) {
        ...
        //显示控制模式
        item = new QTableWidgetItem(controller->mode == MODE_AUTO? tr("自动控制") : tr("手动控制"));
                item->setTextAlignment(Qt::AlignCenter);
...
void ListChild::updateInfoSlot(int index)
{
    Controller *controller = list->at(index);
     tableWidget->item(index, 2)->setText(controller->mode == MODE_AUTO? tr("自动") : tr("手动"));
    QString str = QString().sprintf("%d", controller->angle);
    tableWidget->item(index, 3)->setText(str);
    //更新通讯状态
